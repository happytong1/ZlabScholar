...
本地开发服务器已于 2026-07-15 停止：

- 原 PID：`28992`
- `127.0.0.1:5173` 当前没有监听进程。
- 需要继续本地开发时，重新执行 `npm run dev`。
...
- 公网地址：`http://150.158.50.7/`
- SSH 用户：`ubuntu`（密码不得写入项目或本文档）
- 部署根目录：`/data/shentongtong/zlabscholar`
- 当前 release：`/data/shentongtong/zlabscholar/releases/20260715-2106-0d6b304`
- 当前软链接：`/data/shentongtong/zlabscholar/current`
- Web 服务：Ubuntu 24.04 自带的 Nginx `1.24.0`
- Nginx 站点配置：`/etc/nginx/sites-available/zlabscholar`
- Nginx 已设置为开机启动，最后验证状态为 `enabled`、`active`。

生产环境直接由 Nginx 托管 `current/public`，没有在服务器运行 Vite，也不依赖 Node。主页、前端资源、PDF 分段下载及 404 均已从公网验证通过。腾讯云入站规则至少需要保留 TCP 80；SSH 实测来源 IP 为 `119.78.65.253`，TCP 22 建议仅允许 `119.78.65.253/32`，不要长期对所有 IPv4 开放。当前仅配置 HTTP，尚未绑定域名或配置 HTTPS。
...
- 最新 `npm run build`：通过。
- 本地开发服务器：已停止；原 PID `28992` 已结束，`127.0.0.1:5173` 无监听。
- 腾讯云生产站：`http://150.158.50.7/`，公网首页、静态资源和 PDF 下载已验证。
- 腾讯云 Nginx：已启用且运行中；当前 release 为 `20260715-2106-0d6b304`。
...
## 17. 2026-07-19 开发记录（管理员与人员管理）

### 17.1 已确认的产品规则

- 所有访客无需登录即可查看成员、论文和统计信息。
- 系统只有一个管理员账号，不开放注册，也不创建普通成员账号。
- 成员和论文的新增、修改、删除只允许管理员操作。
- 技术方案已确认使用 Node.js 后端、SQLite 数据库、安全 Cookie 会话和加密密码。
...
- 管理员页面现在可以维护实验室人员：
  ...
  - 教师、博士生、研究生分组。
  - 中英文身份、单位、邮箱、研究方向。
  - 个人主页、ORCID、显示顺序和邮箱核验状态。
  - 头像上传、居中裁剪、压缩为 WebP 并保存到数据库。
- 原有 5 名成员会在空数据库首次启动时自动迁入成员表。
- 公开成员首页改为从 `/api/members` 读取数据库数据；接口不可用时保留原始内置资料作为降级显示。
- 公开页面上的“添加论文”和“修改方向”管理入口已移除，避免普通访客操作。
- 修复管理页“退出登录”在较窄桌面窗口中被响应式样式隐藏的问题。
- `scripts/build-sites.js` 现在构建前端并复制真实后端源文件，不再现场生成单一静态服务器脚本。
- Vite 开发服务器将 `/api` 代理到 `127.0.0.1:3000`。

### 17.3 数据与安全注意事项

- 数据库默认位于 `data/zlabscholar.sqlite`；`data/` 已加入 `.gitignore`。
- 当前本地验收数据库使用 `data/dev/zlabscholar.sqlite`。
- 管理员账号只会在数据库没有管理员时，根据 `ADMIN_USERNAME` 和 `ADMIN_PASSWORD` 环境变量初始化。
- 不要把管理员明文密码写入仓库、交接文档或部署脚本。
- 当前头像以压缩后的图片数据保存在 SQLite。课题组人数较少时可用；未来如图片或文件规模扩大，应迁移到对象存储。
- `DELETE /api/admin/members/:id` 当前采用软删除，避免直接抹掉成员记录。

### 17.4 本地启动方式

需要同时启动后端和前端。已有 `data/dev` 数据库时，不需要再次提供初始化密码。

终端一：

```powershell
cd C:\Users\13557\Desktop\zlabscholar
$env:DATA_DIR = "C:\Users\13557\Desktop\zlabscholar\data\dev"
npm run dev:server
```
...
- 公开首页：`http://127.0.0.1:5173/#members`
- 管理页面：`http://127.0.0.1:5173/#admin`
- 后端健康检查：`http://127.0.0.1:3000/api/health`

当前由 Codex 托管的本地进程可能随任务结束而停止，明天继续时按上述方式重新启动即可。

### 17.5 验证结果

- `npm test`：通过，覆盖密码哈希、未登录拦截、登录、退出以及成员新增/编辑/删除接口。
- `npm run build`：通过。
- 生产构建的公开首页、健康检查和未登录管理接口拦截已做冒烟验证。
- 管理页面、退出按钮、人员列表和新增人员表单已在浏览器实际验证，无控制台错误。

### 17.6 尚未完成，明天建议从这里继续

首要下一项是“论文正式管理”：

1. 建立论文、作者和成员关联的数据表，不能继续只按姓名字符串匹配。
2. 将现有 29 篇论文迁入数据库。
3. 在管理员后台实现论文新增、编辑和删除。
4. 新论文未知引用量必须保存为 `null`，页面显示 `—`，不能默认写成 `0`。
5. 接入 PDF 上传、替换和删除，并保留现有论文文件下载能力。
6. 公开论文列表改为读取数据库。

生产环境尚未部署本次后端改造。上线前还需要：

- 为 Node 后端建立长期运行服务。
- Nginx 将 `/api` 反向代理到 Node 服务。
- 安全设置正式管理员密码和数据目录。
- 备份 SQLite 数据库和上传文件。
- 配置 HTTPS 后启用安全 Cookie。

### 17.7 当前版本控制状态

- 今天的修改尚未提交或暂存。
- `HANDOFF.md` 在本次开发前已经存在用户修改，本次仅在文件末尾追加本节，没有覆盖原内容。
- 新增后端文件位于 `server/`。
- 明天开始前先执行 `git status --short`，不要覆盖现有工作区。
